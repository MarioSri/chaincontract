...
- **MilestoneEscrow.sol (319 lines, Solidity 0.8.24).** A single contract
...
- **Pull-payment design.** Approvals accrue to an internal balance map;
  ...
  participants.
- **16 Hardhat tests** (`test/MilestoneEscrow.test.js`) covering creation,
  funding, overpayment, completion, approval, revision, abort/refund,
  disputes, and unauthorized access — all green on every commit.
- **React + web3.js v4 console** (`client/`, TypeScript) that talks to
  ...
  node, with an import-key mode for local demoing.
...
- **Client creates an agreement** with a title, description, the
  ...
  Funding can happen in the same transaction or later.
...
- **Either party disputes**, freezing approvals for off-chain
...
`disputed` flag that freezes approvals while funds remain locked.
Milestones move `Pending → Completed → Approved` independently inside an
active agreement.

1. **Creation.** `createAgreement` computes the total, stores milestone
   titles and amounts, and optionally funds the escrow in the same call.
...
- **Approval-freeze on dispute.** `dispute` does not move funds; it only
  stops approvals from advancing, keeping both parties' claims legible
...
From the console, import one of the hardhat node private keys (printed
on startup — local demo only, never paste real keys into a browser) to
act as the client or the freelancer and walk the full flow:
create → complete → approve → release → withdraw.
...
test/
└── MilestoneEscrow.test.js # 16 Hardhat tests (ESM, chai matchers)
scripts/
...
This is a teaching-grade escrow, not a production payment processor.
...
deployed address and ABI are generated locally, and the demo private
keys are the well-known hardhat-node defaults used only against the
local node. Against adversarial input the contract reverts with custom
